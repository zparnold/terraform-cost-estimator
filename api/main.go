	...
	for _, change := range pf.ResourceChanges {
		...
		if change.Provider == "registry.terraform.io/hashicorp/azurerm" {
			...
			case "azurerm_managed_disk":
				size, err := strconv.ParseInt(change.Change.After.(map[string]interface{})["disk_size_gb"].(string), 10, 64)
				if err != nil {
					// TODO graceful fail on resource but not on whole compute
					klog.Error(err)
					return 0.0, []string{}, []string{}, err
				}
